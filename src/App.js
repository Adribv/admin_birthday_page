...
function App() {
  ...
  const handleDeletePerson = async (personId, personName) => {
    if (window.confirm(`Are you sure you want to delete ${personName}?`)) {
      ...
      // Delete associated contributions (collection)
      const contributionsSnapshot = await getDocs(collection(db, personName));
      contributionsSnapshot.forEach(async (contributionDoc) => {
        await deleteDoc(doc(db, personName, contributionDoc.id));
      });

      fetchPeople(); // Refresh the list of people
      if (selectedPerson === personName) {
        setSelectedPerson(""); // Reset selected person if deleted
        setContributions([]); // Clear contributions display
      }
    }
  };
                  ...
                  {contributions.map((contribution) => (
                    ...
                    >
                      <div>
                        ...
                        <p>
                          Status: {contribution.received ? "Received" : "Not Received"}
                        </p>
                      </div>
                      <div>
                        <button
